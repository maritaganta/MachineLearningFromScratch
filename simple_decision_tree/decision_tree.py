...
def entropy(y):  # y: all class labels
    hist = np.bincount(y)  # calculate number of occurrences of each class
    ps = hist / len(y)
    return -np.sum([p * np.log2(p) for p in ps if p > 0])


class Node:  # store the best split feature and the best split threshold, as well as left and right child or class
...
class DecisionTree:  # criteria: minimum samples that can be left in a node -> leaf and maximum depth of tree
    ...
    def fit(self, X, y):  # grow tree
        """ Grow Tree

        :param X: training labels
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        if (depth >= self.max_depth
                or n_labels == 1
                or n_sample < self._most_common_label(y)):
